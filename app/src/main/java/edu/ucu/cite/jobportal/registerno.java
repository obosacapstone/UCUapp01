...
public class registerno extends AppCompatActivity implements View.OnClickListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_registerno);
...
//                        arrayList_Barangay.add(item4);
    ...
    private void RegisterUser(){
        ...
        if (!lastname.isEmpty() && !firstname.isEmpty() && !middlename.isEmpty() && !email.isEmpty() && email.matches(emailPattern) && !contact.isEmpty() && contact.length() == 11 && !birthdate.isEmpty()
                 && !gender.isEmpty() && !civilstatus.isEmpty()  && !course.isEmpty() && !skills.isEmpty() &&
                !yeargrad.isEmpty()){
            progressDialog.show();
            StringRequest stringRequest = new StringRequest(Request.Method.POST,
                    Constants.URL_REGISTERNO,
